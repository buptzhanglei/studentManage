	...
	<div class="super-content">
		<div class="superCtab">
			<div class="ctab-title clearfix"><h3>课程预设置</h3>
				<!-- <a href="javascript:;" class="sp-column"><i class="ico-mng"></i></a> -->
			</div>
			<div class="ctab-Main">
				<div class="ctab-Main-title">
					<ul class="clearfix">
						<li class="cur"><a href="chooseLesson">详细信息</a></li>
					</ul>
				</div>
						...
						<table border="0" cellspacing="0" cellpadding="0" class="defaultTable defaultTable2">
							<tbody>
								...
								<tr>
									...
									<?php if($item['if_choose'] == 1):?>
									<td class="t_1 btn"><a class="Top">已选</a></td>
								    <?php else:?>
								    <td class="t_1">未选</td>
								    <?php endif;?>
									<td class="t_1">
										<div class="btn" id="modify">
											<a href="#" class="modify" onclick="modify('<?php echo $item["id"];?>')">查看</a>
											<?php if($item['if_choose'] == 1):?>
											<a href="#" class="delete" onclick="deleteLesson('<?php echo $item["id"];?>')">退课</a>
										    <?php else:?>
											<a href="#" class="Top" onclick="chooseLesson('<?php echo $item["id"];?>')">选课</a>
											<?php endif;?>
										</div>
									</td>
							    </tr>
			...
			<div class="aFllink clearfix" style="margin-top: 38px;">
				<table border="0" cellspacing="0" cellpadding="0" class="defaultTable">
					<tbody>
						...
						<tr>
							<th class="t_1">课程名称</th>
							<th class="t_1"><input type="text" value="" style="width:130px" id="name" disabled="true" /></th>
							<th class="t_1">考核方式</th>
							<th class="t_1">
								<select id="assment" name="assment" style="width:140px;height:40px" disabled="true">
						...
						<tr>
							...
							<th class="t_1"><input disabled="true" type="text" id="course_grade" value="" style="width:130px" /></th>
							<th class="t_1">上课时间</th>
							<th class="t_1">
								<select id="time" name="time" style="width:140px;height:40px" onchange="changeTime()" disabled="true">
								    <option value="0">请选择</option> 
						...
						<tr>
							<th>课程状态</th>
							<th>
								<label id="status" disabled readonly>
									<input type="radio" name="status" value="0" id="status_0">
									未选
								    <input type="radio" name="status" value="1" id="status_1">已选
								</label>
							</th>
							<th>上课教室</th>
							<th id="classroom"><select style='width:140px;height:40px' disabled="true"><option>请选择</option></select></th>
...
<script>
    ...
    function makeSelect(data,room_id){
    	var room = "<select id='classroom' name='classroom' style='width:140px;height:40px' disabled='true'>";
    	if(room_id == 0){
    ...
    function searchBar(){
    	...
    	window.location = "chooseLesson?content="+content;
    }
		...
		$.ajax({
        	...
        	success: function(json){
        		if(json['code'] != 0){
        			...
        			$("#status_"+json['data']['status']).attr('checked',true);
        			$("#hiddenId").val($id);
