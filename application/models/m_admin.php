...
class m_admin extends CI_Model {
    ...
    public function getData($user_id,$type,$searchContent=''){
        if($type == 1){ //根据 user_id 获取课程信息
            $sql = "select a.*,b.username from s_class a,s_basic b where a.teacher_id = b.user_id order by a.id asc";
        ...
        if($searchContent != ''){ // 根据 待查询内容，获取课程信息
            $sql = "select a.*,b.username from s_class a,s_basic b where a.teacher_id = b.user_id and name like '%$searchContent%' order by a.id asc";
        }
